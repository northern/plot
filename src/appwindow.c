// https://github.com/jdapena/gtk/blob/master/perf/appwindow.c
...
static void noop_cb(GtkWidget *widget, gpointer data)
{
  printf("OK\n");
}
...
static GtkUIManager* uimanager_new(void)
{
  GtkUIManager *ui;
...
static GtkWidget* menubar_new(GtkUIManager *ui)
{
  return gtk_ui_manager_get_widget(ui, "/MainMenu");
}

static GtkWidget* toolbar_new(GtkUIManager *ui)
{
...
static GtkWidget* content_area_new(void)
{
  GtkWidget *notebook;

  notebook = gtk_notebook_new();

  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), drawing_area_new(), gtk_label_new("First"));
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), drawing_area_new(), gtk_label_new("Second"));
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), drawing_area_new(), gtk_label_new("Third"));

  return notebook;
}

static GtkWidget* status_bar_new(void)
{
...
GtkWidget* get_appwindow(const gchar* title, gint width, gint height)
{
  GtkWidget *window;
  GtkUIManager *ui;
  GtkWidget *vbox;
  ...
  ui = uimanager_new ();
  g_signal_connect_swapped (window, "destroy", G_CALLBACK (g_object_unref), ui);

  vbox = gtk_vbox_new (FALSE/*GTK_ORIENTATION_VERTICAL*/, 0);
